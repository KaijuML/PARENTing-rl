...
class StructureAwareEncoder(EncoderBase):
    ...
    def forward(self, src, lengths=None):
        ...
        emb = [f(x) for f, x in zip(emb_luts, inputs_)]
        
        # we extract the field-names embedding (can be seen a pos embeddings)
        pos = emb[1]
        
        # we concat + mlp all embeddings
        emb = self.embeddings.make_embedding.mlp(torch.cat(emb, 2))
        ...
        return encoder_final, (memory_bank, pos), lengths
